  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const filtered = response.result.products.filter(
      ...
      <div className="m-4 flex items-center justify-between">
        ...
        <div className="flex items-center justify-center">
          ...
          >
            ...
            <Option value="discount_asc">Giảm giá ít </Option>
          ...
          <Row className="flex flex-wrap" gutter={12}>
            {filteredProducts.length > 0 ? (
              filteredProducts.map((product) => (
                <Col
                  className="m-3 flex flex-col items-center justify-center rounded-md bg-white px-4 pb-4 shadow-md"
                  key={product.id}
                  span={5}
                >
